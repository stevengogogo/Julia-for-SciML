---
title: "Session Info"
comments: false
---

- Place: Blocker 220, Texas A&M University ([Map](https://www.google.com/maps/place/Blocker+Bldg,+College+Station,+TX+77840/@30.6194797,-96.344322,17z/data=!3m1!4b1!4m5!3m4!1s0x86468396697a6c2d:0x947f545ba84db9e2!8m2!3d30.6194797!4d-96.3421333))
- Time: 10:00 am - 12:00 pm. October 25, 2022

(*Latest information is on [TAMIDS page](https://sciml.tamids.tamu.edu/2022/10/12/tamids-sciml-lab-tutorial-julia-sciml/)*)

## Presenter

- [Steven Shao-Ting Chiu](https://github.com/stevengogogo) --- Ph.D. Student, Department of Electrical Engineering, Texas A&M University
- Advisor: Dr. Ulisses Braga-Neto
 


## Background and Objectives

Julia (https://julialang.org/) is a generic programming language designed for high-performance computing. It solves the "two language problem" that typically occurs in scientific computing. Julia is dynamically typed like scripting language such as Python and can be compiled into native machine code. Besides, composablility via multiple dispatches makes Julia works well on the integration across packages. SciML (https://sciml.ai/) is an open-source software for scientific machine learning based on the Julia language that combines machine learning and scientific computing by integrating numerous standalone packages. Notebly, Julia is an open-source project under an MIT license.

This worhshop aims to introduce the potential of the Scientific Machine Learning field with Julia programming language. First, we will give an introductory overview of the Julia programming language and explore the Julia SciML ecosystem as an example of its application.

Both sessions will include presentations and hands-on sessions. Prior knowledge of Python is recommended, and participants are encouraged to bring their own laptops.



![](img/julia-logo-color.png){fig-align="center" width=50%} 


## Schedule


|Time|Content|
|---|---|
|25 min|Introduction to the Julia Programming Language|
|25 min|The Julia SciML Ecosystem|
|10 min|Break|
|30 min|Hands-on session with Julia basics|
|30 min|Hands-on session with SciML application|
: Schedule (Total: 2hrs)

## Access to this webpage


![`https://stevengogogo.github.io/Julia-for-SciML`](img/site-QR-code.png){#fig-surus fig-align="center"}

### Hands-on session

[![](https://github.com/stevengogogo/Julia-for-SciML/actions/workflows/ci.yml/badge.svg){fig-align="left"}](https://github.com/stevengogogo/Julia-for-SciML/actions/workflows/ci.yml) 

- Download hands-on tutorials ([Repo](https://github.com/stevengogogo/Julia-for-SciML/tree/hands-on), [zip](https://github.com/stevengogogo/Julia-for-SciML/archive/refs/heads/hands-on.zip))
- View on [![Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/stevengogogo/Julia-for-SciML/hands-on)